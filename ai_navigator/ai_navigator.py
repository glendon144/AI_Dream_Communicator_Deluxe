...
def _load_webmcp_client_class():
    ...
    try:
        ...
        # WebMCP is an optional integration. The relay's stdio path remains
        # available when its HTTP client dependencies are not installed, so a
        # missing optional package must not prevent the main window opening.
        webmcp_debug_log(
            "client_unavailable",
            {"module": str(client_path), "error": str(exc)},
        )
...
class WebMCPActionsPane(QWidget):
    ...
    def __init__(self, browser_pane: BrowserPane):
        ...
        # Keep this label stable: it is the primary WebMCP workflow control.
        self.execute_button = QPushButton("Execute in Browser")
        self.refresh_button.setObjectName("webmcpRefreshButton")
        self.inspect_button.setObjectName("webmcpPreviewButton")
        self.execute_button.setObjectName("webmcpExecuteButton")
        self.status_label = QLabel("Ready.")
        self.status_label.setWordWrap(True)
    ...
    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Stack controls before a narrow side pane can squeeze them away.
        self._top_row.setDirection(
            QBoxLayout.LeftToRight if self.width() >= 620 else QBoxLayout.TopToBottom
        )
        self._call_row.setDirection(
            QBoxLayout.LeftToRight if self.width() >= 520 else QBoxLayout.TopToBottom
        )
        for button in (self.refresh_button, self.inspect_button, self.execute_button):
            _fit_button_label(button)
...
class ProductLauncherPane(QWidget):
    ...
    def launch_product(self):
        ...
        # Packaged sidecars are relocatable executables, so they run from the
