...
def test_queue_pikit_handoff_is_structured_and_atomic(tmp_path):
    path = queue_pikit_handoff(
        tmp_path / "handoffs",
        title="Useful Article",
        body="Clipboard packet",
        source_capture_id=9,
    )
    assert path.suffix == ".json"
    assert not list(path.parent.glob("*.tmp"))
    payload = __import__("json").loads(path.read_text(encoding="utf-8"))
    assert payload["kind"] == "dream_capture"
    assert payload["title"] == "Dream Capture — Useful Article"
    assert payload["body"] == "Clipboard packet"
