...
class CommandProcessor:
    ...
    def _describe_ai_target(self) -> str:
        provider = "unknown"
        model = ""
        base_url = ""

        try:
            if hasattr(self.ai, "get_provider"):
                provider = str(self.ai.get_provider())
        except Exception:
            pass

        client = getattr(self.ai, "_client", None)
        ...
        if not model:
            try:
                model = str(getattr(self.ai, "_default_model", "") or "")
            except Exception:
                pass
    ...
    def ask_question(self, prompt: str) -> str | None:
        ...
        try:
            ...
            self.logger.info(
                f"Sending standalone prompt to AI ({self._describe_ai_target()} max_tokens={max_toks}): {full_prompt}"
            )
    ...
    def _run_ai_query(self, prompt: str, max_toks: int) -> str:
    ...
    def prepare_query_request(
        ...
        return prompt, base_prompt, max_toks
    ...
    def generate_query_reply(
        ...
        *,
        ...
        self.logger.info(
            f"Sending prompt ({self._describe_ai_target()} max_tokens={max_toks}): {prompt}"
        )
        try:
            reply = self._run_ai_query(prompt, max_toks)
        except Exception as e:
            self.logger.error(f"AI query failed: {e}")
            return None
        self.logger.info("AI query successful")
        return reply, base_prompt

    def finalize_query_reply(
        self,
        *,
        selected_text: str,
        current_doc_id: int,
    ...
    def query_ai(
        ...
        """
        prompt, base_prompt, max_toks = self.prepare_query_request(
            selected_text=selected_text,
            current_doc_id=current_doc_id,
            prefix=prefix,
        )
        generated = self.generate_query_reply(
            prompt=prompt,
        ...
        if generated is None:
            return
        reply, base_prompt = generated
        self.finalize_query_reply(
            selected_text=selected_text,
            current_doc_id=current_doc_id,
            reply=reply,
            base_prompt=base_prompt,
            on_success=on_success,
            on_link_created=on_link_created,
            sel_start=sel_start,
            sel_end=sel_end,
        )
...
"""
        ...
        max_toks, _steer = self._choose_length_policy(source_text)
        prompt = f"{prompt_core}\n\nOutput: be concise, concrete, and structured."

        self.logger.info(
            f"Sending DISTILL prompt ({self._describe_ai_target()} max_tokens={max_toks})"
        )
    ...
    def import_opml_from_url(self, url: str, timeout: int = 15) -> int:
        ...
        try:
            ...
            r = requests.get(
                url,
                timeout=timeout,
                headers={"User-Agent": "FunKit/OPML-Importer"},
            )
            r.raise_for_status()
            xml_text = r.text
        except Exception as e:
    ...
    def import_directory(self, directory: str) -> None:
        ...
        import_text_files_from_directory(directory, self.doc_store)
