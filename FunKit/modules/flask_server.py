...
TOKEN_FILE = Path(__file__).parent.parent / "storage" / "funkit_api_token.txt"
DEFAULT_CERT = Path(__file__).parent.parent / "storage" / "funkit.crt"
DEFAULT_KEY = Path(__file__).parent.parent / "storage" / "funkit.key"
DB_PATH = Path(os.getenv("FUNKIT_DB_PATH", str(Path(__file__).parent.parent / "storage" / "documents.db"))).expanduser().resolve()
...
def _load_api_token() -> Optional[str]:
    token = os.environ.get("FUNKIT_API_TOKEN") or os.environ.get("PIKIT_API_TOKEN")
    if token:
        token = token.strip()
        if token:
            return token
...
def _db_connect() -> sqlite3.Connection:
    if not DB_PATH.exists():
        raise FileNotFoundError(f"FunKit database not found: {DB_PATH}")
...
def _ask_native_ai(
    *,
    ...
    if new_doc_id is None:
        raise RuntimeError("FunKit native ASK did not create a new document. Check AI runtime/config.")

    created = _db_get_doc(store.conn, int(new_doc_id))
    updated_source = _db_get_doc(store.conn, source_doc_id)
    return {
        "status": "ok",
        "engine": "funkit_native_ai",
        "db_path": str(DB_PATH),
        "source_doc_id": source_doc_id,
...
def _distill_native_ai(
    *,
    ...
    if new_doc_id is None and after_max > before_max:
        new_doc_id = int(after_max)
    if new_doc_id is None:
        raise RuntimeError("FunKit native DISTILL did not create a new document. Check AI runtime/config.")
...
if __name__ == "__main__":
    ...
    host = os.environ.get("FUNKIT_FLASK_HOST") or os.environ.get("PIKIT_FLASK_HOST", "127.0.0.1")
    cert = Path(os.environ.get("FUNKIT_CERT") or os.environ.get("PIKIT_CERT", str(DEFAULT_CERT)))
    key = Path(os.environ.get("FUNKIT_KEY") or os.environ.get("PIKIT_KEY", str(DEFAULT_KEY)))
